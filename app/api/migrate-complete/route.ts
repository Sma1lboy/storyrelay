  ...
  try {
    ...
    for (const step of migrationSteps) {
      ...
      try {
        ...
        results.push({ step: step.name, status: 'error', error: err.toString() })
      }
    }
    ...
    const expired = stats?.filter(s => new Date(s.round_end) < new Date()).length || 0

    console.log('Migration completed')
    console.log('Statistics:', { totalSubmissions, unprocessed, processed, expired })
